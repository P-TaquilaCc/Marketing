...
@section('content')
    ...
    <div class="text-right" >
        <a href="{{ route('negocio.banneradd') }}" class="btn btn-primary">Agregar Nuevo</a>
    </div>
    ...
    <table class = "table table-striped table-responsive-md" style="margin-top: 40px; border-collapse: collapse">
        ...
        <tbody>
            @foreach ($banners as $banner)
                <tr>
                    <td>{{ $banner-> imagen }}</td>
                    <td class="text-center">{{ $banner-> estado }}</td>

                    <td class="text-center">
                        <a type="button" href="{{ route('negocio.banneredit') }}" class="btn btn-success" style="border-radius: 50%" data-toggle="tooltip" data-placement="top" title="Editar"><i class="fas fa-pen"></i></a>
